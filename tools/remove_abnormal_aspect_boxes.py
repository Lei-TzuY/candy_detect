...
# 长宽比阈值
MIN_ASPECT_RATIO = 0.1  # 最小长宽比 (太扁)
MAX_ASPECT_RATIO = 10.0  # 最大长宽比 (太长)
...
def check_aspect_ratio(width, height):
    ...
    is_normal = MIN_ASPECT_RATIO <= aspect_ratio <= MAX_ASPECT_RATIO
    return is_normal, aspect_ratio

def remove_abnormal_boxes(labels_dir, backup_dir):
    """删除长宽比异常的标记框"""
    stats = {
        'total_files': 0,
        'modified_files': 0,
    ...
    for label_file in label_files:
        ...
        for line in lines:
            ...
            if len(parts) >= 5:
                ...
                is_normal, aspect_ratio = check_aspect_ratio(width, height)
                ...
                if is_normal:
                    ...
                    stats['details'].append({
                        ...
                        'reason': 'too_flat' if aspect_ratio < MIN_ASPECT_RATIO else 'too_tall'
                    })
            else:
                # 格式错误的行，保留
                new_lines.append(line)
...
def generate_report(stats, backup_dir):
    ...
    report_file = DATASET_DIR / f"remove_abnormal_boxes_report_{timestamp}.txt"
    
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("删除长宽比异常标记框报告\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write(f"阈值设置:\n")
        f.write(f"  最小长宽比: {MIN_ASPECT_RATIO} (低于此值视为太扁)\n")
        f.write(f"  最大长宽比: {MAX_ASPECT_RATIO} (高于此值视为太长)\n\n")
        ...
        if stats['details']:
            ...
            for file, boxes in sorted(files_dict.items()):
                ...
                for box in boxes:
                    reason_text = "太扁" if box['reason'] == 'too_flat' else "太长"
                    f.write(f"  - Class {box['class']}: "
                           f"宽={box['width']:.4f}, 高={box['height']:.4f}, "
                           f"长宽比={box['aspect_ratio']:.4f} ({reason_text})\n")
...
def main():
    ...
    # 确认
    print(f"阈值设置:")
    print(f"  最小长宽比: {MIN_ASPECT_RATIO} (宽/高 < {MIN_ASPECT_RATIO} 视为太扁)")
    print(f"  最大长宽比: {MAX_ASPECT_RATIO} (宽/高 > {MAX_ASPECT_RATIO} 视为太长)")
    print()
    
    response = input("确认删除长宽比异常的标记框? (y/n): ")
    if response.lower() != 'y':
        print("已取消")
        return
    ...
    # 删除异常框
    stats = remove_abnormal_boxes(LABELS_DIR, backup_dir)

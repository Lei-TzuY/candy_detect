...
def run_cmd_Popen_fileno():
    time.sleep(1.6)
    print('運行cmd指令：{}'.format('curl -X POST "http://localhost:8080/api/relay/Relay1?value=1"'))
    subprocess.Popen('curl -X POST "http://localhost:8080/api/relay/Relay1?value=1"', shell=True, stdout=None, stderr=None)
#---------------------------------------------------------------------------------------------------------------------
...
# 嘗試關閉自動對焦並設定固定焦距
# cap.set(cv2.CAP_PROP_AUTOFOCUS, 0)  # 關閉自動對焦
# cap.set(cv2.CAP_PROP_FOCUS, 30)     # 設定焦距值 (數值依鏡頭支援)
